    ...
    async with httpx.AsyncClient(timeout=30) as client:
      while len(out)<total:
        ...
        for x in items:
          ...
          cat,skills=classify(title,provider,str(x.get("adres") or ""));amin,amax,aknown=age_range(title)
          out.append({"id":make_id(KEY,str(x.get("exprnNo") or ""),title,provider),"source_key":KEY,"source_name":NAME,"source_url":PAGE,"detail_url":first_url(x),"external_id":str(x.get("exprnNo") or ""),"region":"부산","city":"","title":title,"provider":provider,"place":provider,"address":x.get("adres"),"lat":None,"lon":None,"age_min":amin,"age_max":amax,"age_known":1 if aknown else 0,"category":cat,"skills_json":json.dumps(skills,ensure_ascii=False),"fee":money(x.get("resveCharge")),"status":{"R":"모집중","E":"접수마감","U":"전화접수"}.get(str(x.get("progrsSttus")),"운영정보 확인"),"start_date":str(x.get("exprnBeginDttm") or "")[:10] or None,"end_date":str(x.get("exprnEndDttm") or "")[:10] or None,"apply_start":str(x.get("reqstBeginDttm") or "")[:16] or None,"apply_end":str(x.get("reqstEndDttm") or "")[:16] or None,"reservation_method":{"1":"온라인","2":"방문접수","3":"전화접수"}.get(str(x.get("exprnResveMth")),str(x.get("exprnResveMth") or "")),"phone":x.get("exprnRefrnc"),"raw_json":json.dumps(x,ensure_ascii=False)})
        page+=1
    return out
