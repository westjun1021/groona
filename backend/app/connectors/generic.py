...
async def converted(key,name,page_url,api_url,region,map):
    ...
    for x in rows:
      ...
      provider=str(v("provider") or name);cat,skills=classify(title,provider,str(v("place") or ""),json.dumps(x,ensure_ascii=False));amin,amax=age_range(str(v("target") or "")+" "+title)
      out.append({"id":make_id(key,str(v("external_id") or ""),title,provider),"source_key":key,"source_name":name,"source_url":page_url,"detail_url":str(v("detail") or first_url(x) or ""),"external_id":str(v("external_id") or ""),"region":region,"city":str(v("city") or ""),"title":title,"provider":provider,"place":str(v("place") or provider),"address":str(v("address") or ""),"lat":None,"lon":None,"age_min":amin,"age_max":amax,"category":cat,"skills_json":json.dumps(skills,ensure_ascii=False),"fee":money(v("fee")),"status":str(v("status") or "운영정보 확인"),"start_date":None,"end_date":None,"apply_start":str(v("apply") or "") or None,"apply_end":None,"reservation_method":str(v("reservation") or ""),"phone":str(v("phone") or ""),"raw_json":json.dumps(x,ensure_ascii=False)})
    return out
